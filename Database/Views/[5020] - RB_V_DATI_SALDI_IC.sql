--------------------------------------------------------
--  DDL for View RB_V_DATI_SALDI_IC
--------------------------------------------------------

CREATE OR REPLACE NOFORCE VIEW "RB_V_DATI_SALDI_IC" (
  COD_SCENARIO,
  COD_PERIODO,
  COD_AZIENDA,
  COD_AZI_CTP,
...
SELECT
  ...
  RB_F_TGK_SANITIZE_CODE(GL_KEY_CODE) AS COD_CONTO,
  CASE
  ...
  RB_F_TGK_SANITIZE_CODE(ACCOUNT) AS COD_DEST1,
  CASE
  ...
  RB_F_TGK_SANITIZE_CODE(PROJECT_ID) AS NOTE,
  PROVENIENZA,
  USERUPD,
  DATEUPD
FROM
  ...
  WHERE
		AFFILIATE IS NOT NULL
		OR OBU_AFFILIATE IS NOT NULL
;